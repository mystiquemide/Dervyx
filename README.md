...
Dervyx is a Base investigation agent for launchpad and exchange vetting. Before a token goes live or while early trading unfolds, give it one token and one fixed block window. It traces funding relationships behind observed swap activity, separates known infrastructure roots, and produces a certificate another reviewer can verify by hash.
